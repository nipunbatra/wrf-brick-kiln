...
c-----------------------------------------------------------------------
c
         subroutine alloc_procan(numgrds,numcols,numrows,numlays,doing_csa)
c
c-----------------------------------------------------------------------
c    Modules used:
c-----------------------------------------------------------------------
...
c        doing_csa     L  .TRUE. if this allocation is for CSA
c     Output:  
c
c-----------------------------------------------------------------------
...
c
         if( doing_csa ) then
            allocate( csa_outfile  ( npadom) )
            allocate( iow_csa      ( npadom) )
            allocate( iptrcsa      ( npadom) )
         endif
c
         return
         end subroutine
...
c-----------------------------------------------------------------------
c
         subroutine alloc_procan_cells(lcsa_is_on,numcells,numspecs,numreact)
c
c-----------------------------------------------------------------------
c    Modules used:
c-----------------------------------------------------------------------
...
c    Argument descriptions:
c     Input:  
c        lcsa_is_on L  doing CSA instead of PA
...
c
c-----------------------------------------------------------------------
c   BEGIN SUBROUTINE ALLOC_CSA_CONC
c-----------------------------------------------------------------------
c
         subroutine alloc_csa_conc(numcells,numgrids,numspecs,
     &                icell_orig,jcell_orig,kcell_orig,
     &                icell_extent,jcell_extent,kcell_extent,
     &                iptr_csa_grids)
c
c-----------------------------------------------------------------------
c    Modules used:
...
c    Argument descriptions:
c     Input:
c        numgrids       I  number of CSA subgrids
c        numcells       I  total number of CSA cells
c        numspecs       I  number of CSA "species"
c        icell_orig     I  origin cell in X direction for CSA grid
c        jcell_orig     I  origin cell in Y direction for CSA grid
c        kcell_orig     I  bottom of column for CSA grid
c        icell_extent   I  last cell in X direction for CSA grid
c        jcell_extent   I  last cell in Y direction for CSA grid
c        kcell_extent   I  top of column for CSA grid
c     Output:
c        iptr_csa_grids I  total number of CSA cells
c
c-----------------------------------------------------------------------
c    Argument declarations:
c-----------------------------------------------------------------------
c
         ...
         integer :: numgrids
         integer :: numspecs
         integer :: icell_orig(numgrids)
         integer :: jcell_orig(numgrids)
         integer :: kcell_orig(numgrids)
         integer :: icell_extent(numgrids)
         integer :: jcell_extent(numgrids)
         integer :: kcell_extent(numgrids)
         integer :: iptr_csa_grids(numgrids)
c
c-----------------------------------------------------------------------
c    Local variables:
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
c    
c  --- set pointers for subgrids ---
c
         mveccsa = 0    
         iptr_csa_grids(1) = 1
         numcols = icell_extent(1) - icell_orig(1) + 1
         numrows = jcell_extent(1) - jcell_orig(1) + 1
         numlays = kcell_extent(1) - kcell_orig(1) + 1
         mveccsa = mveccsa + numcols*numrows*numlays*numspecs
         do igrid=2,numgrids
            numcols = icell_extent(igrid) - icell_orig(igrid) + 1
            numrows = jcell_extent(igrid) - jcell_orig(igrid) + 1
            numlays = kcell_extent(igrid) - kcell_orig(igrid) + 1
            iptr_csa_grids(igrid) = iptr_csa_grids(igrid-1) +
     &                            numcols*numrows*numlays*numspecs
            mveccsa = mveccsa + numcols*numrows*numlays*numspecs
         enddo
c
         allocate( csaconc( mveccsa ) )
         allocate( csaavrg( mveccsa ) )
c
         return
         end subroutine
c
c-----------------------------------------------------------------------
c   END SUBROUTINE ALLOC_CSA_CONC
c-----------------------------------------------------------------------
c
      end Module
