...
c
         ...
         mvec4d_ems = DBLE(mvsa3d_ems) * (soap3_ntotsp)
         mvrowa     = MAXVAL(numrows(1:numgrds))
         mvcola     = MAXVAL(numcols(1:numgrds))
...
c
         allocate( ptname(soap3_ntotsp) )
         allocate( ptnameout(ntotsp) )
         allocate( ptdepname(2*ntotsp) )
         allocate( lsamap(soap3_ntotsp) )
         allocate( lsagas(ntotsp) )
         allocate( loutsa(soap3_ntotsp) )
         allocate( wtkoh (soap3_ntotsp) )
         allocate( wtmir (soap3_ntotsp) )
         allocate( yhrates(soap3_ntotsp), ylrates(soap3_ntotsp) )
...
c
         ...
         mvecdry     = mvsa2d * notimespc
         mvec4d      = DBLE(mvsa3d) * DBLE(ntotsp)
         mvec4d_ems  = DBLE(mvsa3d_ems) * DBLE(soap3_ntotsp)
         mvrowa      = MAXVAL(numrows(1:numgrds))
         mvcola      = MAXVAL(numcols(1:numgrds))
         mvec2a      = mvcola * mvrowa
         mvec3a      = mvcola * mvrowa*MAXVAL(numlays(1:numgrds))
...
c
         if( lcdfout .OR. lcsa ) allocate( ddmdesc(ntotsp) )
         return
c
         end subroutine
c
