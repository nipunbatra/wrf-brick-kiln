...
c 
      ...
      do j=1,ntotsp
          ...
          ispec(1,nspcout) = 'D'
          ispec(2,nspcout) = '_'
          ptdepname(nspcout)(1:10) = 'D_'//ptname(j)
          if( is_srfmod ) then
              ispec(1,nspcout) = 'S' 
              ispec(2,nspcout) = '_' 
              ptdepname(nspcout)(1:10) = 'S_'//ptname(j)
          endif
          read(ptname(j),'(10A1)') (ispec(i,nspcout),i=3,10)
      enddo
      do j=1,ntotsp
          nspcout = nspcout + 1
          ispec(1,nspcout) = 'W'
          ispec(2,nspcout) = '_'
          ptdepname(nspcout)(1:10) = 'W_'//ptname(j)
          if( is_srfmod ) then
             ispec(1,nspcout) = 'V' 
             ispec(2,nspcout) = '_' 
             ptdepname(nspcout)(1:10) = 'V_'//ptname(j)
          endif
          read(ptname(j),'(10A1)') (ispec(i,nspcout),i=3,10)
      enddo
c
