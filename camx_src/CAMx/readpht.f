...
c
  ...
  11        continue
            ...
            albcl(ialb) = tmp3(2)
            trncl(itrn) = tmp3(3)
            if (ozcl(iozn).lt.250. or. ozcl(iozn).gt.600.) goto 7005
c
            do ihght = 1,NHGHT
              line = line + 1
...
c
 7005 continue
      write(iout,'(//,A)') 'ERROR in READPHT:'
      write(iout,'(/,a)') ' Ozone Column is outside a reasonable range'
      write(iout,'(a,a)') '       Found: ',ozcl(iozn)
      write(iout,'(a,a)') ' Expecting Dobson Units between 250-600'
      write(iout,'(2a,/,a)') ' Make sure you are using the correct ',
     &                       'version of the TUV processor for this',
     &                    ' version of CAMx and chemistry mechanism.'
      call camxerr()
