     ...
     &                  iflag,igrd,ncol,nrow,nlay,nrads,ngaspc,nspc,
     &                  nsen,delt,dx,
     &                  dy,idfin,depth,entrn,dilut,tempk,press,species,
     &                  conc,lreadtop,rhoscl,ctop,lvupsolv,ptrtop,fluxes,
     &                  fluxtmp,isaptr,ipa_xy,ipa_lay,iproc_id)
      use filunit
      use chmstry
      use bndary
      use procan
...
c     is performed on the regular model species (iflag=1), RTRAC species
c     (iflag=2), or source apportionment tracers (iflag=3).
c     This version also performs vertical transport of sensitivities 
c     in the regular model call (iflag = 1) if DDM is enabled.
c                          
...
c       12/23/24   Fix logical tests for loading core and PT top cons
c  
c     Input arguments:
c        iflag             call ID (1=reg, 2=RTRAC, 3=SAT)
c        igrd              grid index
c        ncol              number of columns
c        nrow              number of rows
c        nlay              number of layers
c        nrads             number of radicals
c        ngaspc            number of gas species
...
c        lreadtop          top con read from input file 
c        rhoscl            density scaling above top of model (unitless)
c        ctop              concentrations at model top
c                               (umol/m3 if from top layer, ppm if read)
c        lvupsolv          3-D flag to use upstream donor solver
c        ptrtop            DDM sensitivities at model top (like ctop)
...
c
      if( iflag .EQ. 1 ) call zeros(fluxtmp,m1*m2*m3*nspc)
c
...
c
      ...
      if (lzppm) entrnfac = -1.
      numspcs = nspc
      if( iflag .GT. 1 ) numspcs = nsaspc

      if( iproc_id .LE. 1 ) then
        if( iflag .eq. 1 ) then
          write(*,'(a20,$)') 'z advection ......'
        elseif (iflag .eq.2 ) then
          write(*,'(a20,$)') 'RT z advection ...'
        else
...
c
            ...
            enddo
            if (lreadtop) then    !<--- from argument, core vs. PT specific
              convfac = 1.
              if ((iflag.LT.3 .AND. ispc .LE. ngaspc) .OR. !<---- this is not right for a SA/RT call
     &            (iflag.EQ.3 .AND. lsagas(ispc)))         !<---- this is not right for a SA/RT call
     &          convfac = densfac*(273./tempk(i,j,nlay))*
     &                            (press(i,j,nlay)/1013.)
              c1d(nlay+1) = convfac*ctop(i,j,ispc)
            else
              c1d(nlay+1) = rhoscl(i,j)*ctop(i,j,ispc) !<--- needs if/else
            endif
c
c================================DDM Begin============================
c
     ...
     &                DBLE(m1)*DBLE(m2)*DBLE(m3)*DBLE(ioff-1)
                ...
                enddo
                if (lreadtop) then    !<--- from argument, DDM-specific
                  convfac = 1.
                  if (ispc .LE. ngaspc)
     &              convfac = densfac*(273./tempk(i,j,nlay))*
     &                                (press(i,j,nlay)/1013.)
                  sen1d(nlay+1,isen) = convfac*ptrtop(i,j,ioff)
                else
                  sen1d(nlay+1,isen) = rhoscl(i,j)*ptrtop(i,j,ioff) !<--- neds if/else
                endif
              enddo
            endif
c
