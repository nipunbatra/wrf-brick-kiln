...
c
        if( ltrace .OR. ((lddm .OR. lhddm) .AND. lddmcalc(igrd)) ) then
           call emiss(igrd,mxp,myp,mzp,i0,j0,ia,iz,ja,jz,
     &              ntotsp,soap3_ntotsp,nlu,1,densfac,lsamap,.TRUE.,
...
c
     ...
     &             ltopcon,ctop(iptr1lay(igrd)),pttop,
     &             fluxes(1,igrd),ipsa3d(igrd),
...
c
     ...
     &             1,igrd,ncol(igrd),nrow(igrd),
     &             nlay(igrd),nrad,ngas,nspec,MAX(1,ntotsp),deltat(igrd),
     &             deltax(1,igrd),deltay(igrd),
     &             idfin(iptr2d(igrd)),depth(iptr3d(igrd)),
     &             entrn(iptr3d(igrd)),dilut(iptr3d(igrd)),
     &             tempk(iptr3d(igrd)),press(iptr3d(igrd)),
     &             spname,conc(iptr4d(igrd)),ltopcon,rhoscl,
     &             ctop(iptr1lay(igrd)),lvupsolv,pttop,fluxes(1,igrd),
     &             fluxtmp,ipsa3d(igrd),ipacl_2d(iptr2d_full(igrd)),
     &             ipacl_3d(iptr3d_full(igrd)),iproc_id )
          call flush(6)
          call flush(iout)
c
...
c
     ...
     &                .FALSE.,pttop,pttop,fluxdum,ipsa3d(igrd),
     &                ipacl_3d(iptr3d_full(igrd)),iproc_id )
c
c  --- call routine with tracer species arrays, send dummy
...
c
                 call zadvec(mxp,myp,mzp,i0,j0,ia,iz,ja,jz,ibcon,
     &                2,igrd,ncol(igrd),nrow(igrd),
     &                nlay(igrd),0,nrtgas,ntotsp,1,deltat(igrd),deltax(1,igrd),
     ...
     &                .FALSE.,rhoscl,pttop,lvupsolv,pttop,fluxdum,fluxtmp,
     &                ipsa3d(igrd),ipacl_2d(iptr2d_full(igrd)),
     &                ipacl_3d(iptr3d_full(igrd)),iproc_id )
             else
                 lrdtop = .FALSE.
                 if (lsa_iortc .OR. ltopcon) lrdtop = .TRUE. 
                 call zadvec(mxp,myp,mzp,i0,j0,ia,iz,ja,jz,ibcon,
     &                3,igrd,ncol(igrd),nrow(igrd),
     &                nlay(igrd),0,0,ntotsp,1,deltat(igrd),
     &                deltax(1,igrd),deltay(igrd),
     &                idfin(iptr2d(igrd)),depth(iptr3d(igrd)),
     &                entrn(iptr3d(igrd)),dilut(iptr3d(igrd)),
     &                tempk(iptr3d(igrd)),press(iptr3d(igrd)),
     &                ptname,ptconc(ipsa3d(igrd)),lrdtop,rhoscl,
     &                pttop,lvupsolv,pttop,fluxdum,fluxtmp,
     &                ipsa3d(igrd),ipacl_2d(iptr2d_full(igrd)),
     &                ipacl_3d(iptr3d_full(igrd)),iproc_id )
c
