...
c        08/10/21 -gy-       Added RACM2s21 (Mech 9)
c        11/03/23 -cemery-   Removed Mechanism 6 (CB05)
...
c
      ...
      real dcprec(NPREC_S) ! change in SOA precursor concentrations [ppm]
...
c
      ...
      real flu(NLU)
c
      integer ncol_csa(MXPADOM)
      integer nrow_csa(MXPADOM)
      integer nlay_csa(MXPADOM)
c
c-----Entry point
c
      call flush(6)
...
c
      aero_flag = 0
      if( naero.GT.0 .AND. aeropt.EQ.'CF') then
         aero_flag = 1
...
c
      if( lcsa ) then
         do icsa_grid = 1,npadom
            ncol_csa(icsa_grid) = i_ne(icsa_grid) - i_sw(icsa_grid) + 1
            nrow_csa(icsa_grid) = j_ne(icsa_grid) - j_sw(icsa_grid) + 1
            nlay_csa(icsa_grid) = t_lay(icsa_grid) - b_lay(icsa_grid) + 1
         enddo
      endif
c
c======================== CSA End =======================
...
c
c  ---- load the CSA values for this cell into 2-D array ---
c
            if( lcsa ) then
               ipa_idx = ipa_cel(i+i0,j+j0,k)
               if( ipa_idx .GT. 0 ) then
                   icsa_grid = ipadom(ipa_idx)
                   icell = ipax(ipa_idx) - i_sw(icsa_grid) + 1
                   jcell = ipay(ipa_idx) - j_sw(icsa_grid) + 1
                   kcell = ipaz(ipa_idx) - b_lay(icsa_grid) + 1
                   call loadcsa_sens(.FALSE.,ncol_csa(icsa_grid),nrow_csa(icsa_grid),
     &                 nlay_csa(icsa_grid),ntotsp,csaconc(iptrcsa(icsa_grid)),
     &                 icell,jcell,kcell,nddmsp,nspec,ngas,nrad,NHETRXN,sddm,convfac)
              endif
           endif
c
c======================== DDM End =======================================
...
c
     ...
     &                                     + con(kB124) + con(kARO2)
                ...
                elseif (idmech.eq.9) then ! RACM2s21
                  con_prec(1) = con(kBEN )
                  con_prec(2) = con(kTOL )
                  con_prec(3) = con(kXYM ) + con(kXYO) + con(kXYP)
                  con_prec(4) = con(kISO )
                  con_prec(5) = con(kAPI ) + con(kLIM)
                  con_prec(6) = con(kSQT )
                endif
                ...
                if (idmech.eq.1 .or. idmech.eq.3 .or. idmech.eq.4) then ! cb6
                  ...
                  con_prec(5) = con(kSVOA)
                  con_prec(6) = con(kISOP)
                  con_prec(7) = con(kTERP)
                  con_prec(8) = 0.0
                  con_prec(9) = con(kSQT )
                elseif (idmech.eq.5) then ! saprc07tc
                  con_prec(1) = con(kBENZ)
     ...
     &                                     + con(kB124) + con(kARO2)
                  ...
                  con_prec(6) = con(kISOP)
                  con_prec(7) = con(kTERP)
                  con_prec(8) = con(kAPIN)
                  con_prec(9) = con(kSESQ)
                elseif (idmech.eq.7) then ! cb7
                  con_prec(1) = con(kBENZ)
                  con_prec(2) = con(kTOL )
                  con_prec(3) = con(kXYL )
...
c
                if ( lddm ) then
                  if (idmech.eq.1 .or. idmech.eq.3 .or. idmech.eq.4) then ! cb6
                    do is = 1, nddmsp
                      ...
                      sen_prec(4*nddmsp+is) = sddm(iptddm(kSVOA)-1+is)
                      sen_prec(5*nddmsp+is) = sddm(iptddm(kISOP)-1+is)
                      sen_prec(6*nddmsp+is) = sddm(iptddm(kTERP)-1+is)
                      sen_prec(7*nddmsp+is) = 0.0
                      sen_prec(8*nddmsp+is) = sddm(iptddm(kSQT )-1+is)
                    enddo
                  elseif (idmech.eq.5) then ! saprc07tc
                    do is = 1, nddmsp
                      sen_prec(         is) = sddm(iptddm(kBENZ)-1+is)
     ...
     &                                      + sddm(iptddm(kARO2)-1+is)
                      ...
                      sen_prec(5*nddmsp+is) = sddm(iptddm(kISOP)-1+is)
                      sen_prec(6*nddmsp+is) = sddm(iptddm(kTERP)-1+is)
                      sen_prec(7*nddmsp+is) = sddm(iptddm(kAPIN)-1+is)
                      sen_prec(8*nddmsp+is) = sddm(iptddm(kSESQ)-1+is)
                    enddo
                  elseif (idmech.eq.7) then ! cb7
                    do is = 1, nddmsp
                      sen_prec(         is) = sddm(iptddm(kBENZ)-1+is)
                  ...
                  elseif (idmech.eq.9) then ! RACM2s21
                    do is = 1, nddmsp
                      sen_prec(         is) = sddm(iptddm(kBEN )-1+is)
                      sen_prec(  nddmsp+is) = sddm(iptddm(kTOL )-1+is)
                      sen_prec(2*nddmsp+is) = sddm(iptddm(kXYM )-1+is)
     &                                      + sddm(iptddm(kXYO )-1+is)
     &                                      + sddm(iptddm(kXYP )-1+is)
                      sen_prec(3*nddmsp+is) = sddm(iptddm(kIVOA)-1+is)
                      sen_prec(4*nddmsp+is) = sddm(iptddm(kSVOA)-1+is)
                      sen_prec(5*nddmsp+is) = sddm(iptddm(kISO )-1+is)
                      sen_prec(6*nddmsp+is) = sddm(iptddm(kLIM )-1+is)
                      sen_prec(7*nddmsp+is) = sddm(iptddm(kAPI )-1+is)
                      sen_prec(8*nddmsp+is) = sddm(iptddm(kSQT )-1+is)
                    enddo
                  endif
                endif
...
c
     ...
     &                  hr_nxy9,hr_pan9,hr_xo_dum,
     &                  dtchem,water(i,j,k),atm,O2,
     &                  CH4,H2,tcell,ldark(i,j),con,avgcnc,avit(i,j),
     &                  hddmjac9,nddmsp,sddm,NOXID,avgsen,
     &                  (lddm.AND.lddmcalc(igrd)),
     &                  nirrrxn,rrxn_irr,ltrapirr)
               endif
c
...
c
     ...
     &                        rk,avgcnc,sddm,ierr)
              elseif (idmech.eq.9) then
                call hddmchem(hddmjac9,ebirxn9,ebirate9,ddmrate9,
     &                        nreact,ngas,ngas,
     &                        nddmsp,nrateddm,ntermddm,nhddm,
     &                        iprate,ipterm,wfterm,iphddm,
     &                        dtchem,water(i,j,k),atm,O2,CH4,H2,
...
c
                if ( lddm ) then
                  do is = 1, nddmsp
                    sen_CG(         is) = sddm(iptddm(kACG2)-1+is)
                    sen_CG(  nddmsp+is) = sddm(iptddm(kACG1)-1+is)
                    sen_CG(2*nddmsp+is) = sddm(iptddm(kBCG2)-1+is)
                    sen_CG(3*nddmsp+is) = sddm(iptddm(kBCG1)-1+is)
                  enddo
                endif
c
c======================== DDM End =======================
c
                call soachem(iout,tcell,pcell,dtchem,rk,
     &                       avgox,con_prec(:NPREC_S),dcprec(:NPREC_S),
...
c
     ...
     &                      (idmech.eq.7 .and. kSQT.lt.nspec+1))) then
                    do is = 1, nddmsp
                      sddm(iptddm(kSQT)-1+is) = sen_prec(8*nddmsp+is)
                    enddo
                  endif
                  do is = 1, nddmsp
                    sddm(iptddm(kIVOA)-1+is) = sen_prec(3*nddmsp+is)
                    sddm(iptddm(kSVOA)-1+is) = sen_prec(4*nddmsp+is)
                    sddm(iptddm(kACG2)-1+is) = sen_CG(         is)
                    sddm(iptddm(kACG1)-1+is) = sen_CG(  nddmsp+is)
                    sddm(iptddm(kBCG2)-1+is) = sen_CG(2*nddmsp+is)
                    sddm(iptddm(kBCG1)-1+is) = sen_CG(3*nddmsp+is)
                  enddo
                endif
c
c======================== DDM End =======================
c
                if (.NOT.((idmech.eq.5 .and. kSESQ.lt.nspec+1) .OR.
     &                    (idmech.eq.7 .and. kSQT.lt.nspec+1)))
     &            con(kSQT) = amax1( con_prec(9), bdnl(kSQT) )
                con(kIVOA) = amax1( con_prec(4), bdnl(kIVOA) )
                con(kSVOA) = amax1( con_prec(5), bdnl(kSVOA) )
                con(kACG2) = con_CG(1)
                con(kACG1) = con_CG(2)
                con(kBCG2) = con_CG(3)
                con(kBCG1) = con_CG(4)
...
c
            if( lcsa ) then
               ...
               if( ipa_idx .GT. 0 ) then
                   ...
                   jcell = ipay(ipa_idx) - j_sw(icsa_grid) + 1
                   kcell = ipaz(ipa_idx) - b_lay(icsa_grid) + 1
                   call loadcsa_sens(.TRUE.,ncol_csa(icsa_grid),nrow_csa(icsa_grid),
     &                 nlay_csa(icsa_grid),ntotsp,csaconc(iptrcsa(icsa_grid)),
     &                 icell,jcell,kcell,nddmsp,nspec,ngas,nrad,NHETRXN,sddm,convfac)
              endif
           endif
...
c
     ...
     &                           aerph,ajno2(i,j,k),con_NV,con,
     &                           dcprec,convfac,dtchem,aero_dt(igrd),
...
c-----Calculate volume concentrations of fine & coarse primary particles
              ...
              if(khoa  < nspec+1)vfine = vfine + con(khoa)/roprt(khoa)
...
c
               if( lsulfate .OR. lnitrate .OR. lsoa 
     &                              .OR. lprimary .OR. lmercury ) then
...
c
     ...
     &                                                    nn,ldark(i,j))
               elseif( idmech.EQ.9 ) then
                 call cpamech9(rrxn_irr,rk,dtfact,MXREACT,patmp,ntotsp,
     &                                                    nn,ldark(i,j))
               else
                 write(iout,'(//,a)') 'ERROR in CHEMDRIV:'
