...
c-----------------------------------------------------------------------
c
c ---- save the dates coming in for later chack ---
c
      bnddate_in = bnddate
      bndtim_in = bndtim
c
...
c
      if( is_netcdf_ibc ) then
        if (iproc_id .EQ. 0) call ncf_readbnd(time,date,did_update_ibc)
        if( lmpi ) then
...
c
        ...
        if (date .EQ. bnddate_in .AND. ABS(time-bndtim_in) .LT. 0.01) then
...
c
      if (did_update_ibc .OR. (date .EQ. bnddate_in .AND. ABS(time-bndtim_in) .LT. 0.01)) then
        if (iproc_id .eq. 0) then
          if (ltrace) then
...
c
     ...
     &                      .AND. ABS(time-sa_bndtim) .LT. 0.01) then
            ...
            if( lsa_iortc ) then
               if( is_netcdf_iortc ) then
                  call ncf_rd_sa_tcfile()
               else
                   if( date .EQ. sa_bnddate
     &                      .AND. ABS(time-sa_bndtim) .LT. 0.01) then
                      call rd_sa_tcfile()
...
c
      if (ltopcon) then
        if( is_netcdf_itc ) then
          if( iproc_id .EQ. 0) call ncf_readtop(time,date,did_update_itc)
          if( lmpi )
...
c============================= DDM Begin ===============================
c
        if( did_update_itc .OR. (date .EQ. topdate .AND. ABS(time-toptim) .LT. 0.01)) then
          if (iproc_id .EQ. 0) then
            if ((lddm.OR.lhddm) .AND. nbcddm .GT. 0) then
              call rdtcddm()
