...
c     Sets the rate constant for hydrolysis of organic nitrate:
c       NTR2       -> HNO3
c       INTR       -> HNO3
c     Sets the rate constant for heterogeneous reaction of SO2:
c       SO2        -> SULF
c     Sets the rate constant for heterogeneous reaction of GLY/MGLY to CGLY:
c       GLY       -> CGLY
c       MGLY      -> CGLY
c
...
c        07/07/2022  - gy   -     Force gas-phase k(N2O5+H2O) = 1E-22 
c        03/21/2024  - lhuang -   Added het GLY/MGLY -> CGLY
c        05/17/2024  - cemery -   Removed CMU PM option
c
c     Input arguments:
...
c
     ...
     &                   cstar2 = 1000.
      ...
      real, parameter :: mwGLY  = 0.058         ! MW of GLY [kg/mol]
      real, parameter :: mwMGLY = 0.072         ! MW of MGLY [kg/mol]
      ...
      real, parameter :: diff_GLY = 93.         ! Diffusivity of GLY at 298 K [Torr-cm2/s]; Tang et al. 2015, ACP
      ...
      real :: cbarGLY                           ! mean molecular speed of GLY [m/s]
      real :: cbarMGLY                          ! mean molecular speed of MGLY [m/s]
      ...
      real :: dc_GLY                            ! diffusion coefficient of GLY [m2/s]
      real :: dc_MGLY                           ! diffusion coefficient of MGLY [m2/s]
      real :: gamma_N2O5                        ! uptake coefficient of N2O5
      real :: gamma_SO2                         ! uptake coefficient of SO2
      real :: gamma_GLY                         ! uptake coefficient of GLY
      real :: gamma_MGLY                        ! uptake coefficient of MGLY
      real :: y_clno2                           ! product yield for ClNO2
      real :: kgasn2o5                          ! gas phase N2O5 + H2O rate constant

      real :: t1, t2
      real*8 :: r1, r2, rdenom
      real :: rscl
      real :: khet0, khet1, khet2, khet5, khet6, khet7
...
c======================== DDM End =======================
c
c-----k N2O5 + H2O from IUPAC data sheet NOx33 converted [cm3/molecule-sec] -> [1/ppm-hr]
c
      kgasn2o5 = 1.0E-22*(298.0/tcell)*(pcell/1013.0)*8.87e16
c
c-----No PM chemistry:
...
c
      if (ihetrxn(3).gt.0 .or. ihetrxn(4).gt.0) then
        if (aeropt.eq.'CF') then
          ctoa = con(khoa)  + con(kaoa2) + con(kaoa1)
     &                      + con(kboa2) + con(kboa1)
     &                      + con(kaoa0) + con(kboa0)
          if (lvbs) then
     ...
     &            +con(kpfp0)+con(kpfp1)+con(kpfp2)+con(kpfp3)+con(kpfp4)
        ...
        elseif (rh.ge.0.4) then
          ...
          t1 = fp * khon0 * (rh - 0.2) / 0.2
          t2 = fp * khon20 * (rh - 0.2) / 0.2
        endif
        if (ihetrxn(3).gt.0) rk(ihetrxn(3)) = t1
        if (ihetrxn(4).gt.0) rk(ihetrxn(4)) = t2
c
c======================== DDM Begin =======================
c
        if ( lddm .and. aeropt.eq.'CF' ) then
          do ip = 1, nfam
            stoa = sddm(ip,khoa)
     &           + sddm(ip,kaoa2) + sddm(ip,kaoa1)
     &           + sddm(ip,kboa2) + sddm(ip,kboa1)
     &           + sddm(ip,kaoa0) + sddm(ip,kboa0)
            sfp = 0.0
            if (ctoa.ge.0.0001) then
              sfp = ( fp - ( 
...
c
      ...
      cbarGLY = sqrt( 8.0*Rgc*tcell / (Pi*mwGLY) )          ! [m/s]
      dc_GLY = (diff_GLY/7.6e6) * (1013.25/pcell) * (tcell/298.)**1.75 ! [m2/s]
      !dc_GLY = (0.0855e-4) * (1013.25/pcell) * (tcell/298.)**1.75  ! [m2/s] based on CMAQ

      cbarMGLY = sqrt( 8.0*Rgc*tcell / (Pi*mwMGLY) )          ! [m/s]
      dc_MGLY = (diff_GLY/7.6e6) * (1013.25/pcell) * (tcell/298.)**1.75 ![m2/s]
...
c       Add GLY heterogeneous
        gamma_GLY = 0.001                                ! from CMAQ AEROSOL_CHEMISTRY.F
        gamma_MGLY = 0.09 * gamma_GLY                    ! from CMAQ AEROSOL_CHEMISTRY.F

        khet6 = saden / ( diam * 0.5e-6 / dc_GLY +
     &                    4. / ( cbarGLY * gamma_GLY ) )    ! [1/s]
        khet6 = MIN( khet6, rtau )                          ! check against upper limit [1/s]

        khet7 = saden / ( diam * 0.5e-6 / dc_MGLY +
     &                    4. / ( cbarMGLY * gamma_MGLY ) )    ! [1/s]
        khet7 = MIN( khet7, rtau )                          ! check against upper limit [1/s]
c
c======================== DDM Begin =======================
c
        if ( lddm ) then !bk - broken
...
c
 800  continue
      rk(ihetrxn(1)) = kgasn2o5 + ( khet1 * h2s / H2O )     ! add gas-phase rate; [1/ppm-hr]
      if (ihetrxn(2).gt.0) rk(ihetrxn(2)) = khet2 * h2s / con(khcl)
      if (ihetrxn(5).gt.0) rk(ihetrxn(5)) = khet5 * h2s     ! [1/hr]
      if (ihetrxn(6).gt.0) rk(ihetrxn(6)) = khet6 * h2s     ! [1/hr]
      if (ihetrxn(7).gt.0) rk(ihetrxn(7)) = khet7 * h2s     ! [1/hr]
