     ...
     &                   lreadtop,ctop,ptrtop,fluxes,isaptr,
     &                   ipa_cel,iproc_id)
      use filunit
      use chmstry
      use bndary
...
c     Output arguments:
c        conc              species concentrations (umol/m3)
c        ctop              concentrations in top layer (umol/m3)
c        ptrtop            tracer/DDM concentrations in top layer (umol/m3)
...
c
      if (.NOT.lreadtop) then   !<-- this needs to be an argument for reg, SA, RT
        do l = 1,nspc
          do j = 1,m2
            do i = 1,m1
