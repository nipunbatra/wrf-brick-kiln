...
c        03/24/21      Added Mechanism 7 (CB7)
c        09/02/21      Added Mechanism 9 (RACM2)
c        11/03/23      Removed Mechanism 6 (CB05)
c
c     Input arguments:
...
c
     ...
     & Number_of_DEP_Species_Groups,Dep_Species_Groups,
...
c
      namelist /CSA_Control/
     & CSA_File_Root,Number_of_CSA_Domains,Within_CAMx_Grid,
     & CSA_Beg_I_Index,CSA_End_I_Index,CSA_Beg_J_Index,CSA_End_J_Index,
     & CSA_Beg_K_Index,CSA_End_K_Index,Number_of_Rate_Const_Groups,
     & Rate_Const_Groups,Number_of_Rate_Term_Groups,Rate_Term_Groups
c
c======================== Probing Tool End =============================
c
...
c      
      ...
      integer   nemiss,cbdate,cedate,inp,ii,iifroot,nopen,igrd,icsa_grid
      logical   lexist, lacross
      logical   lmech1_ok, lmech2_ok, lmech3_ok, lmech4_ok
      logical   lmech5_ok, lmech6_ok, lmech7_ok, lmech8_ok
      logical   lmech9_ok, lmech10_ok
...
c
      ...
      if (ibyr.lt.1000) then
        write(iout,'(//,a)') 'ERROR in READNML:'
        write(iout,'(a)') 'Simulation start year must be 4-digit YYYY'
        write(iout,'(a,i5)') 'Year:  ',ibyr
        call camxerr()
      endif
...
c
      ...
      if (ieyr.lt.1000) then
        write(iout,'(//,a)') 'ERROR in READNML:'
        write(iout,'(a)') 'Simulation end year must be 4-digit YYYY'
        write(iout,'(a,i5)') 'Year:  ',ieyr
        call camxerr()
      endif
...
c
     ...
     &    Probing_Tool .NE. STRIRR .AND.
     &    Probing_Tool .NE. CSA) then
          write(iout,'(//,A)') 'ERROR in READNML:'
...
c
     ...
     &                                         'another mechanism.'
      ...
      if( idmech.EQ.2 .AND. .NOT. lmech2_ok ) then
         write(iout,'(//,a)') 'ERROR in READNML:'
         write(iout,'(/,2A)') ' You have selected a probing tool that is ',
     &               'currently not supported with Mechanism 2.'
         write(iout,'(/,2A)') ' Turn off probing tools or select ',
     &                                         'another mechanism.'
         call camxerr()
      endif
     ...
     &                                         'another mechanism.'
      ...
      if( idmech.EQ.8 .AND. .NOT. lmech8_ok ) then
         write(iout,'(//,a)') 'ERROR in READNML:'
         write(iout,'(/,2A)') ' You have selected a probing tool that is ',
     &               'currently not supported with Mechanism 8.'
         write(iout,'(/,2A)') ' Turn off probing tools or select ',
     &                                         'another mechanism.'
         call camxerr()
      endif
      if( idmech.EQ.9 .AND. .NOT. lmech9_ok ) then
         write(iout,'(//,a)') 'ERROR in READNML:'
         write(iout,'(/,2A)') ' You have selected a probing tool that is ',
     &               'currently not supported with Mechanism 9.'
         write(iout,'(/,2A)') ' Turn off probing tools or select ',
     &                                         'another mechanism.'
         call camxerr()
      endif
...
c
c  --- CSA is not allowed for MPI ---
c
      if( lmpi .AND. lcsa ) then
        write(iout,'(//,a)') 'ERROR in READNML:'
        write(iout,'(/,1X,2A,/,A)') 'CSA is not allowed with MPI. Please',
     &                 ' turn off MPI and use OMP',' parallelization instead.'
        call camxerr()
      endif
c
c  --- Certain PM chemistry and probing tools are not allowed ---
c
      if( (aeropt.EQ.'INERT' .OR. 
     &     lvbs) .AND. tectyp.NE.'NONE      ') then
          write(iout,'(//,a)') 'ERROR in READNML:'
          write(iout,'(/,2A)') ' Probing Tools are ',
     &     'not supported with the INERT or VBS Aerosol Schemes.'
...
c
     ...
     &          ' namelist group when probing tools is set to '//
     &                                    tectyp(:istrln(tectyp))//'.'
            read(inp,CSA_Control,END=7100,ERR=7101)
            flrtsa = CSA_File_Root
      endif
c
c======================== Probing Tool End =============================
c 
...
c
     ...
     &            'Number of BC groups                 : ',nbcddm
         ...
         write(idiag,'(A,I10)') 'Number of Deposition groups: ',ndepddm
         if( ndepddm .GT. 0 ) then
            do i = 1,ndepddm
              write(idiag,'(24X,i3,'': '',A)') i,depddmsp(i)
            enddo
         else
            write(idiag,'(29X,A)') 'No Depostion species modeled.'
         endif
         write(idiag,'(A,I10)') 
...
c
         do icsa_grid=1,npadom
            jj = istrln(flrtsa)
            filtmp = flrtsa
            write(filtmp(jj+1:),'(a,i2.2,a)') '.csa.grd',icsa_grid,'.nc'
            csa_outfile(icsa_grid) = filtmp
            nopen = nopen + 1
            write(action,'(A,I3)') 'Opening output file for CSA grid: ',icsa_grid
            call ncf_createfile(filtmp,action,iow_csa(icsa_grid))
            write(iout,9000) 'Gridded output file for CSA domain: ',icsa_grid
            write(iout,9002) filtmp(1:jj+13)
         enddo
c
c  --- allocate arrays for pointers ---
c
         call alloc_ddm_species(nspec)
      endif
c
