...
c
c     Revision History:
c       08/01/25   Version 7.32 released (see Release_notes_7.32)
c       06/24/24   Version 7.30 released (see Release_notes_7.30)
...
c
      ...
      integer icsa_grid, igrid_mod, ncol_csa, nrow_csa, nlay_csa, wrtcsa_date
      integer iproc_id, nlayav, ndps, num_dims
      real inptim,emstim,ozntim,bndtim,toptim,wrttim,endtim,wrt_csa_tim
      character*200 action
      character*20 version
      character*8 chtime, chdate
      logical lupdtdep(MXGRID), loutspec(MXSPEC+MXTRSP)
c
      data version /'CAMx v7.32, 08-01-25'/
c
c-----Entry point
c
...
c
      if( lcsa ) then
         ...
         whr = aint(time/100.)
         wmn = amod(time,100.)
         wrtcsa_tim = 100.*(whr + aint((wmn + 60.)/60.)) +
     &                                     amod((wmn + 60.),60.)
         if (wrtcsa_tim.ge.2400.) then
            wrtcsa_tim = wrtcsa_tim - 2400.
            wrtcsa_date = wrtcsa_date + 1
            if( MOD(wrtcsa_date,1000) .GT. 365 ) then
               if( MOD(INT(wrtcsa_date/1000),4) .EQ. 0 ) then
                   if( MOD(wrtcsa_date,1000) .EQ. 367 )
     &                 wrtcsa_date = (INT(wrtcsa_date/1000)+1)*1000 + 1
               else
                   wrtcsa_date = (INT(wrtcsa_date/1000)+1)*1000 + 1
               endif
            endif
         ...
         do igrd=1,ngrid
            call csa_reset(igrd,ncol(igrd),nrow(igrd),nlay(igrd),
     &               nspec,ngas,nrad,NHETRXN,ipacl_3d(iptr3d_full(igrd)))
         enddo
         csaavrg = 0.
...
c
          if( lcsa ) then
             do icsa_grid=1,npadom
                ...
                write(action,'(2A,I2)') 'Writing data to output file for ',
     &                                                  'CSA grid: ',icsa_grid
                call ncf_wrt_data_tstep(action,iow_csa(icsa_grid),ntotsp)
                call ncf_wrt_data_species_csa(action,iow_csa(icsa_grid),4,
     &                          ncol_csa,nrow_csa,nlay_csa,nlay_csa,nlay_csa,
     &                          ntotsp,ptname,csaavrg(iptrcsa(icsa_grid)),
     &                          height(iptr3d(igrid_mod)))
             enddo
          endif
c
c======================== CSA End =======================
...
c
      if( lcdfout .OR. lcsa ) call ncf_closefiles()
c
