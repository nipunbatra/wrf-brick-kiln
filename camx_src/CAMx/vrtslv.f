...
c       08/30/22   Fixed sensitivity array dims to be consistent
c                  with ZADVEC and avoid accessing memory out-of-bounds
c
c     Input arguments: 
...
c
     ...
     &                pn(k)*zrm(k)*entrn(k)*sens(k+1,isen)
            do k = 2,nlay-1
              ls = ls + 1
              lrr = lrr + 1
              rr(lrr) = -pn(k)*zrp(k-1)*entrn(k-1)*sens(k-1,isen) +
     &                  sens(k,isen)*(1. - pn(k)*
     &                           (zrm(k-1)*entrn(k-1) -
     &                            zrp(k)*entrn(k) +
     &                            dilut(k))) +
     ...
     &                          zrp(k)*entrn(k) + 
     &                          dilut(k))) +
     &                zrm(k)*entrn(k)*sens(k+1,isen)*dt/depth(k)
            ls = ls + 1
          enddo
        endif
c
