...
c        05/16/24       Added CGLY implementation of GLY & MGLY reactive uptake
c                       forming non-volatile SOA
c
c     Input arguments:
...
c        dcprec              change in SOA precursor concentrations [ppm]
...
c
          ...
          con(kaoa0) = con(kaoa0) + cNV(1) * convfac * mwaoa0
          con(kboa0) = con(kboa0) + cNV(2) * convfac * mwboa0
c
c --- Scheme for reactive uptake of GLY and MGLY
c     forming non-volatile SOA. Estimate the anthro/bio fractions.
c
c     dcprec contains SOA precursor concentration change [ppm]
c            1 - BNZA; 2 - TOLA; 3 - XYLA; 4 - IVOA
c            5 - SVOA; 6 - ISP ; 7 - TRP ; 8 - APN; 9 - SQT
c
          fra = min (1.0, (
     &          ( dcprec(1) + dcprec(2) + dcprec(3) ) /
     &          ( dcprec(1) + dcprec(2) + dcprec(3) + dcprec(6) ) ) )
          frb = 1.0 - fra

          con(kaoa0) = con(kaoa0) + fra * con(kcgly) * convfac * mwaoa0
          con(kboa0) = con(kboa0) + frb * con(kcgly) * convfac * mwboa0
          con(kcgly) = bdnl(kcgly)
c
c========================= Process Analysis Begin ======================
c
          if ( ldoipr ) then
            cipr(IPR_OAERO,ipa_cel,kaoa0) = cipr(IPR_OAERO,ipa_cel,kaoa0)
     &                                         + con(kaoa0) - cold(kaoa0)
            cipr(IPR_OAERO,ipa_cel,kboa0) = cipr(IPR_OAERO,ipa_cel,kboa0)
     &                                         + con(kboa0) - cold(kboa0)
          endif
c
c========================= Process Analysis End ========================
c
...
c
          if (lddm) then
            do ip = 1, nfam
              sddm(ip,jdpAOA0) = sddm(ip,jdpAOA0) 
     &              + scNV(ip,1) * convfac * mwaoa0
     &                + fra * sddm(ip,jdpCGLY) * convfac * mwaoa0
              sddm(ip,jdpBOA0) = sddm(ip,jdpBOA0) 
     &              + scNV(ip,2) * convfac * mwboa0
     &                + frb * sddm(ip,jdpCGLY) * convfac * mwboa0
              sddm(ip,jdpCGLY) = 0.0
            enddo
          endif
c
...
c
          ...
          cg(1)  = con(kacg2) * convfac * mwsoap(1)
          cg(2)  = con(kacg1) * convfac * mwsoap(2)
          cg(3)  = con(kbcg2) * convfac * mwsoap(3)
          cg(4)  = con(kbcg1) * convfac * mwsoap(4)
          soacon(1) = con(kaoa2)
          soacon(2) = con(kaoa1)
          soacon(3) = con(kboa2)
          soacon(4) = con(kboa1)
          preOM = con(khoa) / mwhoa
     &          + con(kaoa0) / mwaoa0
     &          + con(kboa0) / mwboa0
c
c========================= DDM Begin ===================================
...
c
          ...
          enddo
          aphot(NSOAP+1) = con(kaoa0) * fphot
          con(kaoa0) = con(kaoa0) - aphot(NSOAP+1)
          aphot(NSOAP+2) = con(kboa0) * fphot
          con(kboa0) = con(kboa0) - aphot(NSOAP+2)
c
...
c
     ...
     &                                          igrdchm,ichm,jchm,kchm
              ...
              write(iout,*) ' CG = ',con(kacg2),con(kacg1),
     &                               con(kbcg2),con(kbcg1)
              write(iout,*) ' SOA = ',con(kaoa2),con(kaoa1),
     &                                con(kboa2),con(kboa1),
     &                                con(kaoa0),con(kboa0)
              write(iout,*) ' APHOT = ',(aphot(ispc),ispc=1,NSOAP+2)
              write(iout,*) ' APOLY = ',(apoly(ispc),ispc=1,NSOAP)
              write(iout,*) ' HOA = ',con(khoa)
              do ip = 1, nfam
                write(iout,*) ' SI(',ip,') = ',
     &                               (sddm(ip,jdpACG2+is-1),is=1,NSOAP),
     &                              (sddm(ip,jdpAOA2+is-1),is=1,NSOAP),
     &                               sddm(ip,jdpAOA0),sddm(ip,jdpBOA0),
     &                                                 sddm(ip,jdpHOA)
              enddo
              call camxerr()
            endif
...
c========================== DDM End ====================================
c
          con(kacg2) = amax1(cg(1)/convfac/mwsoap(1),bdnl(kacg2))
          con(kacg1) = amax1(cg(2)/convfac/mwsoap(2),bdnl(kacg1))
          con(kbcg2) = amax1(cg(3)/convfac/mwsoap(3),bdnl(kbcg2))
          con(kbcg1) = amax1(cg(4)/convfac/mwsoap(4),bdnl(kbcg1))
          con(kaoa2) = amax1(soacon(1),bdnl(kaoa2))
          con(kaoa1) = amax1(soacon(2),bdnl(kaoa1))
          con(kboa2) = amax1(soacon(3),bdnl(kboa2))
          con(kboa1) = amax1(soacon(4),bdnl(kboa1))
          con(kaoa0) = amax1(con(kaoa0),bdnl(kaoa0))
          con(kboa0) = amax1(con(kboa0),bdnl(kboa0))
c
c========================= Process Analysis Begin ======================
c
          if ( ldoipr ) then
            cipr(IPR_OAERO,ipa_cel,kacg2) = cipr(IPR_OAERO,ipa_cel,kacg2)
     &                                 + (con(kacg2)-cold(kacg2))*convfac
            cipr(IPR_OAERO,ipa_cel,kacg1) = cipr(IPR_OAERO,ipa_cel,kacg1)
     &                                 + (con(kacg1)-cold(kacg1))*convfac
            cipr(IPR_OAERO,ipa_cel,kbcg2) = cipr(IPR_OAERO,ipa_cel,kbcg2)
     &                                 + (con(kbcg2)-cold(kbcg2))*convfac
            cipr(IPR_OAERO,ipa_cel,kbcg1) = cipr(IPR_OAERO,ipa_cel,kbcg1)
     &                                 + (con(kbcg1)-cold(kbcg1))*convfac
            cipr(IPR_OAERO,ipa_cel,kaoa2)=cipr(IPR_OAERO,ipa_cel,kaoa2)
     &                                 + con(kaoa2)-cold(kaoa2)
            cipr(IPR_OAERO,ipa_cel,kaoa1)=cipr(IPR_OAERO,ipa_cel,kaoa1)
     &                                 + con(kaoa1)-cold(kaoa1)
            cipr(IPR_OAERO,ipa_cel,kboa2)=cipr(IPR_OAERO,ipa_cel,kboa2)
     &                                 + con(kboa2)-cold(kboa2)
            cipr(IPR_OAERO,ipa_cel,kboa1)=cipr(IPR_OAERO,ipa_cel,kboa1)
     &                                 + con(kboa1)-cold(kboa1)
            cipr(IPR_OAERO,ipa_cel,kaoa0)=cipr(IPR_OAERO,ipa_cel,kaoa0)
     &                                 + con(kaoa0)-cold(kaoa0)
            cipr(IPR_OAERO,ipa_cel,kboa0)=cipr(IPR_OAERO,ipa_cel,kboa0)
     &                                 + con(kboa0)-cold(kboa0)
...
c
     ...
     &                        sddm(ip,jdpBOA0)/wtmol(12)/convfac*1.e-6
            enddo
            call camxerr()
          endif
        endif
...
c
     ...
     &                               AMAX1(bdnl(ispc),con(ispc))
         ...
         if ( lsvoc .and. .not.lvbs ) then
           delcon(5,kaoa2) = -apoly(1)
           delcon(5,kaoa1) = -apoly(2)
           delcon(5,kboa2) = -apoly(3)
           delcon(5,kboa1) = -apoly(4)
           do ispc = 1, NSOAA
             delcon(5,kaoa0) = delcon(5,kaoa0) + apoly(ispc)
           enddo
           do ispc = NSOAA+1, NSOAP
             delcon(5,kboa0) = delcon(5,kboa0) + apoly(ispc)
           enddo
           delcon(6,kaoa2) = -aphot(1)
           delcon(6,kaoa1) = -aphot(2)
           delcon(6,kboa2) = -aphot(3)
           delcon(6,kboa1) = -aphot(4)
           delcon(6,kaoa0) = -aphot(5)
           delcon(6,kboa0) = -aphot(6)
         endif
         if ( lradm ) then
