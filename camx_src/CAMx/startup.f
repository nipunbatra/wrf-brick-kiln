...
c
      ...
      logical       l_exist_ivoc, l_exist_hoa_xx, l_exist_pap
c
c-----Entry point
c
      lfirst_call = .TRUE.
...
c   --- make sure that the parameters are large enough ---
c
          icheck = MAX(ntotsp,soap3_ntotsp)
          iparm = MXTRSP
          cparm = 'MXTRSP'
          if( icheck .GT. iparm ) goto 7000
          icheck = nlay(1)
...
c
      if (lptsrc) then
         ...
         call alloc_tracer_sapnts(nptsrc,soap3_ntotsp)
      else
         call alloc_grid_ptsrc(MAX(1,MXPTSRC))
...
c
      ...
      if( lvbs .AND. LVBSPREPROC ) then
         ...
         enddo
         if( .NOT. l_exist_ivoc .OR. .NOT. l_exist_hoa_xx ) then
            write(iout,'(//,a)') 'ERROR in STARTUP:'
...
c
        ...
        if( .NOT. lcsa ) call alloc_procan_ipa(ngrid,ncol,nrow,nlay,tectyp,0)
      endif
c
c===================== Process Analysis End ============================
c
c-----Call routine to initialize CSA subdomain
c
      if( lcsa ) then
         call pagrids()
         call speccsa( )
         call wrt_csa_hdr(endtim,enddate)
      endif
c
c=================== CSA End ==========================
c
