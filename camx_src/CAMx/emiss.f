     ...
     &                 nspc,nspecem,nlu,ndpspc,densfac,lspmap,lprobe,nsrc,
...
c        lspmap              emissions source species map
...
c
     ...
     &    hstktmp, tstktmp, vstktmp
      real*8  dmass
      real    pnt_emiss_svoa(MXLAYER), area_emiss_hoa
      real    pnt_emiss_hoa(MXLAYER)
      real    pnt_emiss_soap3(IDX_SOAP_POA_OP:IDX_SOAP_POA_BB,MXLAYER)
      integer idx_svoa, idx_hoa
      integer iems, isoa, numhoa

      real, allocatable, dimension(:,:) :: sa_pnt_emiss_hoa
      real, allocatable, dimension(:,:) :: sa_pnt_emiss_svoa
c
...
c
      ...
      idx_hoa = -9
c
c----allocate data structures for SA HOA emissions for points ---
c
      if( luse_soap3 .AND. lprobe .AND. lsoa ) then
         numhoa = nptcls(idxipt(ITRHOA))-iemcls(idxipt(ITRHOA))+1
         allocate( sa_pnt_emiss_hoa(iemcls(idxipt(ITRHOA)):nptcls(idxipt(ITRHOA)),MXLAYER)  )
         allocate( sa_pnt_emiss_svoa(iemcls(idxipt(ITRSVA)):nptcls(idxipt(ITRSVA)),MXLAYER) )
      endif
c
...
c
      if (larsrc) then
c
c-----If doing SOAP3, call routine to update emissions for POA evap ---
c
        if( luse_soap3 .AND. .NOT. lprobe ) then
             call soap3_evap_area( igrd, m1, m2, m3, nlay_ems,
     &              soap3_evap_factors, tempk, aremis, nspecem)
        endif
        if( lprobe .AND. luse_soap3 .AND. lsoa ) then
             call SA_soap3_evap_area( igrd, m1, m2, m3, nlay_ems,
     &              soap3_evap_factors, tempk, aremis, soap3_ntotsp)
        endif
c
...
c$omp&          pnt_emiss_svoa,pnt_emiss_hoa,pnt_emiss_soap3,idx_svoa,
c$omp&          iems,isoa)
c
...
c
  ...
  17      do k = kb,kt
          ...
          enddo
c
c  --- if doing SOAP3, call routine to update emissions for evaporative ---
c
          if( luse_soap3 .AND. .NOT. lprobe ) then
             pnt_emiss_soap3 = 0.
             pnt_emiss_svoa = 0.
             ...
             do iems=1,nemspc
                do isoa=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                   if( emspcname(iems) .EQ. soap_emiss_names(isoa) ) then
                       do k = kb,kt
                           pnt_emiss_soap3(isoa,k) = pnt_emiss_soap3(isoa,k) + 
     &                                                pttrace(n,iems)*pfrac(k)
                       enddo
                   endif
                enddo
                if( emspcname(iems)(1:5) .EQ. 'POA ' 
     &                          .OR. emspcname(iems)(1:5) .EQ. 'HOA ' ) then
                   do k = kb,kt
                      pnt_emiss_soap3(IDX_SOAP_POA_AV,k) = 
     &                             pnt_emiss_soap3(IDX_SOAP_POA_AV,k) +
     &                                             pttrace(n,iems)*pfrac(k)
                    enddo
                endif
             enddo
             call soap3_evap_pts( n, kb, kt, idx_svoa,
     &                 idx_hoa, soap3_evap_factors, tempk1d, pnt_emiss_soap3,
     &                                       pnt_emiss_svoa, pnt_emiss_hoa)
          endif
          if( luse_soap3 .AND. lprobe .AND. lsoa ) then
             call SA_soap3_evap_pts( n, kb, kt, numpts, soap3_ntotsp,
     &                   iemcls(idxipt(ITRHOA)),nptcls(idxipt(ITRHOA)), 
     &                     iemcls(idxipt(ITRSVA)),nptcls(idxipt(ITRSVA)), 
     &                              soap3_evap_factors, tempk1d, pttrace,
     &                                 sa_pnt_emiss_hoa,sa_pnt_emiss_svoa)
          endif
          do 40 l = 1,nspc
            imod = lspmap(l)
            if( imod .LE. 0 ) goto 40
c
c  --- adjust SOAP3 species for evap ---
c
            ...
            do k = kb,kt
              ...
              if( .NOT. lprobe ) then
                  if( luse_soap3 .AND. imod .EQ. idx_svoa ) then
                      dmass = dmass+DBLE(pnt_emiss_svoa(k)*deltat*1e6)
                  elseif( luse_soap3 .AND. imod .EQ. idx_hoa ) then 
                      dmass = DBLE(pnt_emiss_hoa(k)*deltat*1e6)
                  else
                      dmass = DBLE(pttrace(n,imod)*deltat*1e6*pfrac(k))
                  endif
              endif
              if( lprobe ) then
                 if( lsoa .AND. luse_soap3 ) then
                     if( imod .GE. iemcls(idxipt(ITRSVA)) 
     &                             .AND. imod .LE. nptcls(idxipt(ITRSVA)) ) then
                         dmass = DBLE(sa_pnt_emiss_svoa(l,k)*deltat*1e6*pfrac(k))
                     endif
                     if( imod .GE. iemcls(idxipt(ITRHOA))
     &                             .AND. imod .LE. nptcls(idxipt(ITRHOA)) ) then
                        dmass = DBLE(sa_pnt_emiss_hoa(l,k)*deltat*1e6*pfrac(k))
                     endif
                 else
                   dmass = DBLE(pttrace(n,imod)*deltat*1e6*pfrac(k))
                 endif 
...
c
      if( luse_soap3 .AND. lprobe .AND. lsoa ) then
         deallocate( sa_pnt_emiss_hoa )
         deallocate( sa_pnt_emiss_svoa )
      endif
c
