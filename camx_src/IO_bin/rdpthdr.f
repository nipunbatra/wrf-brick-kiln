...
c
     ...
     &                                        ptspc = 'FORM      '
           do isoa=1,NUM_SOAP_SPECIES
              if( ptspc .EQ. soap2_spec_names(isoa) ) 
     &                            ptspc = soap3_spec_names(isoa)  
           enddo  
           if( ptspc(1:4) .EQ. 'HOA ') ptspc = 'POA_AV    '
...
c   
           do 20 l = 1,nspec 
             if( lvbs .AND. LVBSPREPROC ) then
                if( ptspc .EQ. soap_emiss_names(IDX_SOAP_POA_OP) ) then
                    if( LVBSPREPROC ) then
                      lemmap(kpap_c(0)) = lpt
     ...
     &                        lpt,ptspc,' mapped to model species PAP0-PAP5 (VBS)'
                    ...
                    if( luse_soap3 ) then
                      lemmap(ksvoa) = lpt
                      write(idiag,'(a,i5,2x,2a)') 'Point source species ',
     &                                lpt,ptspc,' mapped to model species SVOA'
                    endif
                endif
                if( ptspc .EQ. soap_emiss_names(IDX_SOAP_POA_BB) ) then
                    if( LVBSPREPROC ) then
