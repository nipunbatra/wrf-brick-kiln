...
c
     ...
     &        nx,ny,nz,iproj,idum,i,j,n,l,lic,nlayer,num_dims
...
c
      do 20 l = 1,nspec
        ...
        do 15 lic = 1,nicspc
          ...
          if (icspc(1:5) .EQ. 'HNO2 ') icspc = 'HONO      '
          if (icspc(1:5) .EQ. 'HCHO ' .AND. kHCHO .EQ. nspec+1) 
     &                                              icspc = 'FORM      '
          if( icspc(1:4) .EQ. 'POA ' ) icspc = 'HOA       '
          do isoa=1,NUM_SOAP_SPECIES
             if( icspc .EQ. soap2_spec_names(isoa) ) 
     &                           icspc = soap3_spec_names(isoa)
          enddo
