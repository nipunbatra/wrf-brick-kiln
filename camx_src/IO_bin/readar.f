...
c
      ...
      real poa_em(IDX_SOAP_POA_OP:IDX_SOAP_POA_BB,ncol,nrow)
...
c
        if( lvbs .AND. LVBSPREPROC ) then
           do isoa=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
              if( arspcname .EQ. soap_emiss_names(isoa) ) then 
                 do i=1,ncol
                   poa_em(isoa,i,1) = argrid(i,2)
                   poa_em(isoa,i,nrow) = argrid(i,nrow-1)
                 enddo
                 do j=1,nrow
                   poa_em(isoa,1,j) = argrid(2,j)
                   poa_em(isoa,ncol,j) = argrid(ncol-1,j)
                 enddo
                 do j=1+buffer_offset,nrow-buffer_offset
                    do i=1+buffer_offset,ncol-buffer_offset
                      poa_em(isoa,i,j) = poa_em(isoa,i,j) + argrid(i,j)/(60.*dtems)
                    enddo
                 enddo
              endif
           enddo
...
c
        do ispc=1,nemspc
           if( arspcname(1:4) .EQ. 'HOA ') arspcname = 'POA_AV    '
           if( arspcname(1:4) .EQ. 'POA ' .AND. luse_soap3 ) arspcname = 'POA_AV    '
