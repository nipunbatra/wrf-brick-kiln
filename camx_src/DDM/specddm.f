...
c
      if( lbndry ) then
          nbdic = 5 * nbcddm + nicddm + ndepddm
      else
          nbdic = nbcddm + nicddm + ndepddm
      endif
...
c
     ...
     &                                    srcnam,idxddm,0,0,lout,0.0)
          enddo
c
c  --- loop over all of the DDM depostion species ---
c
          do iddm = 1,ndepddm
             cinflu = depddmsp(iddm)
             srcnam = 'DEP'
c
c  --- call routine to fill the family of species names ---
c
...
c
      if (lchem .AND. aeropt.EQ.'CF') then
         ...
         jdpmap(jdpHCL ) = khcl
         jdpmap(jdpACG2 ) = kacg2
         jdpmap(jdpACG1 ) = kacg1
         jdpmap(jdpBCG2 ) = kbcg2
         jdpmap(jdpBCG1 ) = kbcg1
         ...
         jdpmap(jdpPCL ) = kpcl
         jdpmap(jdpAOA2) = kaoa2
         jdpmap(jdpAOA1) = kaoa1
         jdpmap(jdpBOA2) = kboa2
         jdpmap(jdpBOA1) = kboa1
         jdpmap(jdpAOA0) = kaoa0
         jdpmap(jdpBOA0) = kboa0
         jdpmap(jdpHOA ) = khoa
...
cgwilson
c------------- Added mapping for MCGLY Sun Mar 16 12:21:03 PDT 2025
c
         jdpmap(jdpCGLY) = kcgly
cgwilson
