...
c     This is also called when probing tools is CSA
c
...
c
      call alloc_procan_cells(lcsa,npa_cels,nspec,nreact)
c
c-----write header for table of cells to the diag file ---
c
      if( lcsa ) then
         write(idiag,'(//,15X,A,/)')
     &        '*** Model Cells Treated by CSA ***'
      else
         write(idiag,'(//,15X,A,/)')
...
c
 7001 continue
     ...
     &                     'contain cells that are included in a nest.'
         ...
         write(iout,'(//,A)') 'ERROR in PAGRIDS:'
         write(iout,'(1X,2A)') 'CSA Analysis domain cannot ',
     &                     'contain cells that are included in a nest.'
         write(iout,'(1X,A,I3)') 'Please redefine CSA domain.'
         call camxerr()
      endif
c
 9999 continue
