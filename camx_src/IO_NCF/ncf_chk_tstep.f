...
c
      if( ignore_date ) then
         ...
         call clean_date_time(date_time_tflag)
         call clean_date_time(date_time_etflag)
      endif
...
c
      if( lcheck_begtime .AND. lcheck_endtime ) goto 9999
      goto 111
c
 222  continue
...
c
 9999 continue
      ...
      subroutine clean_date_time(date_time)
        ! handle datetime past end of year

        real, intent(inout) :: date_time
        integer :: nday, year,jday

        year=int(date_time)/1000
        jday = int(date_time) - year*1000
        ...
        if (jday>nday) then
          ...
          if (jday>nday+365) then
            print*,'dont go too far!'
            print*,date_time, year, jday, nday
            stop
          endif

          year = year+1
          jday = 1
          date_time = year*1000 + jday + mod(date_time, 1.0)
