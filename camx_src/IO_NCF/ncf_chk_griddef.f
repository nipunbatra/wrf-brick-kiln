...
c
      ...
      if ( lchk_layers ) then
        if( nlays_in .NE. nlay(igrd) ) lcheck_ok = .FALSE.
      else
        if (nlays_in .GT. 1 .AND. nlays_in .NE. nlay(igrd) )
     &    lcheck_ok = .FALSE.
      endif
...
c
 7001 continue
     ...
     &                                 ncols_match,',',nrows_match,')'
      write(iout,'(A,I4)') 'Number of layers: ',nlay(igrd)
     ...
     &                                     ycell_in,',',ycell_in,')'
      if( lchk_cells) write(iout,'(A,I4,A,I4,A)') 'Number of cells: (',
     &                                 ncols_in,',',nrows_in,')'
      write(iout,'(A,I4)') 'Number of layers: ',nlays_in
      call camxerr()
c
 7002 continue
      write(iout,'(//,a)') 'ERROR in NCF_CHK_GRIDDEF:'
