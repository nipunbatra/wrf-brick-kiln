...
c
      ...
      integer       buffer_offset, itmp, jtmp, isoa, idx_poa, this_idx
c
      real, allocatable, dimension(:,:,:)   ::  argrid
      real, allocatable, dimension(:,:,:,:) ::  poa_em
...
c
      ...
      allocate( poa_em(IDX_SOAP_POA_OP:IDX_SOAP_POA_BB,ncol,nrow,nlays_in) )
      poa_em = 0.0
c
...
c
      ...
      if( luse_soap3 ) then
         do ispc=1,nemspc
           if( emspcname(ispc) .EQ. soap_emiss_names(IDX_SOAP_POA_AV) ) then
             idx_poa = ispc
           endif 
         enddo
      endif
...
c
     ...
     &                                               data_count,argrid)
          ...
          this_idx = ispc
          if( idx_poa .GT. 0 .AND. this_var(1:4) .EQ. 'POA ') then
             this_idx = idx_poa 
          endif 
c
...
c
     ...
     &                           i+buffer_offset,j+buffer_offset,k,' Species: ',ispc
                   ...
                   aremis(i,j,k,this_idx) = aremis(i,j,k,this_idx) + 
...
c
      if( lvbs .AND. LVBSPREPROC ) then
          do isoa=1,IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
     ...
     &                        i+buffer_offset,j+buffer_offset,k,' Species: ',ispc
                         ...
                         poa_em(isoa,i,j,k) = argrid(itmp,jtmp,k)
                      enddo
                   enddo
                enddo
             endif
...
c
     ...
     &                       ( poa_em(IDX_SOAP_POA_OP,:,:,:) * poa_op_ef(ibin)
     &                       + poa_em(IDX_SOAP_POA_GV,:,:,:) * poa_gv_ef(ibin)
     &                       + poa_em(IDX_SOAP_POA_DV,:,:,:) * poa_dv_ef(ibin) ) /
     &                                              (60.*dtems)
              endif
              if( emspcname(ispc) .EQ. spname(kpcp_c(ibin)) ) then
                     aremis(:,:,:,ispc) = aremis(:,:,:,ispc) +
     &                      ( poa_em(IDX_SOAP_POA_MC,:,:,:) * poa_mc_ef(ibin)  ) /
     &                                              (60.*dtems)
              endif
              if( emspcname(ispc) .EQ. spname(kpfp_c(ibin)) ) then
                aremis(:,:,:,ispc) = aremis(:,:,:,ispc) +
     &                      ( poa_em(IDX_SOAP_POA_BB,:,:,:) * poa_bb_ef(ibin) ) /
     &                                              (60.*dtems)
              endif
           enddo
         enddo
