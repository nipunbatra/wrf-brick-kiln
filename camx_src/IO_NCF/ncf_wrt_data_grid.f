...
c     08/21/23   --cemery--     Fixed X,Y 1-D arrays to remove nested
c                               buffer cells
c
c-----------------------------------------------------------------------
c    Include files:
...
c
      ...
      if( igrd .EQ. 1 ) then
         ...
         allocate( darray_1d(ncol(igrd)-2) )
         ibegcel = 2
         iendcel = ncol(igrd)-1
         buffer_offset = 1
      endif
      ...
      do icol=ibegcel,iendcel
        this_val = this_val + 1
        darray_1d(this_val) = DBLE((grid_xorig + 
...
c
      ...
      if( igrd .EQ. 1 ) then
         ...
         allocate( darray_1d(nrow(igrd)-2) )
         jbegcel = 2
         jendcel = nrow(igrd)-1
      endif
      this_val = 0
      do irow=jbegcel,jendcel
        this_val = this_val + 1
        darray_1d(this_val) = DBLE((grid_yorig + 
