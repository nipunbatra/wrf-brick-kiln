...
c
      if( luse_soap3 ) then
         do ipoa=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
            idx_poa(ipoa) = -9
            do ispc = 1,nemspc
               if( emspcname(ispc) .EQ. soap_emiss_names(ipoa) )
     &                                       idx_poa(ipoa) = ispc
            enddo
         enddo
      endif
...
c
     ...
     &                                               data_count,emispts)
          if( ierr .NE. NF_NOERR) goto 7001
          if( ierr .NE. NF_NOERR) goto 7001
          this_idx = lemmap(ispc)
          if( idx_poa(IDX_SOAP_POA_AV) .GT. 0 
     &                .AND. this_var(1:4) .EQ. 'POA ') then
             this_idx = idx_poa(IDX_SOAP_POA_AV)
          endif
c
c-----Put the data into the global array ----
c
          do ipt=1,nptsrc_files(ifile)
             pntems(idx_start+ipt,this_idx) = 
     &          pntems(idx_start+ipt,this_idx) + emispts(ipt)/(60.*dtems)
          enddo
c
c ---- next species ---
c
      enddo
c
c  ---- if doing SOAP3, loop over the list of SOAP emissions spcies ---
c
      do ispc=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
c
c  --- load data into temporary arrray ---
c
          this_var = soap_emiss_names(ispc)
          ierr = nf_inq_varid(iunit, this_var, this_varid)
          if( ierr .NE. NF_NOERR) cycle
          ierr = nf_get_vara_real(iunit,this_varid,data_start,
     &                                               data_count,emispts)
          if( ierr .NE. NF_NOERR) goto 7001
          if( idx_poa(ispc) .LE. 0 ) cycle
c
c-----Put the data into the global array ----
c
          do ipt=1,nptsrc_files(ifile)
             pntems(idx_start+ipt,idx_poa(ispc)) = 
     &           pntems(idx_start+ipt,idx_poa(ispc)) + emispts(ipt)/(60.*dtems)
          enddo
c
c ---- next species ---
c
