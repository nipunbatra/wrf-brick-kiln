...
c
      logical lfound, lfound_hoa, lfound_svoa, lfound_poa
      integer ifle, igroup, iounit
c
      integer, parameter :: num_elem = 5
      integer :: idx_elem(num_elem)
      integer idx_start, numpts, n, l, lpt, i, j, k, idum
      integer idat1, idat2, nseg, izone, nx, ny, nz
      real tim1, tim2, orgx, orgy, utmx, utmy, dx, dy
c
      integer isoa, idx_hoa, idx_svoa
c
      data ptfil /'PTSOURCE  '/
...
c 
     ...
     &                                        ptspc = 'FORM      '
           do isoa=1,NUM_SOAP_SPECIES
              if( ptspc .EQ. 'POA ' ) cycle
              if( ptspc .EQ. soap2_spec_names(isoa) ) then
                  ptspc = soap3_spec_names(isoa)
              endif
           enddo
           if( ptspc(1:4) .EQ. 'HOA ') ptspc = 'POA       '
           if( ptspc(1:4) .EQ. 'POA ' .AND. luse_soap3 ) ptspc = 'POA_AV    '
c
c  --- check if species is not modelled ---
   
           lfound = .FALSE.
           do l = 1,nspec 
             if(spname(l)(1:4) .EQ. 'HOA ' ) idx_hoa = l
             if(spname(l)(1:5) .EQ. 'SVOA ' ) idx_svoa = l
     ...
     &                   ' mapped to model species ',l,spname(l) 
               ...
               if( .NOT. lfound ) then
                 ...
                 lemmap(l) = nemspc
                 idxpts(igroup,ifle,lpt) = nemspc
                 lfound = .TRUE.
               endif
             endif
           enddo
           if( .NOT. lfound .AND. luse_soap3 .AND. lsoa ) then
               do j=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                   if(ptspc .EQ. soap_emiss_names(j)) then
                      lfound_poa = .FALSE.
                      do k=1,nemspc
                         if( ptspc .EQ. emspcname(k) ) then
                              lfound_poa = .TRUE.
