...
c
      ...
      integer numcls
      integer igroup
      real    emscls(soap3_ntrcls,nox,noy,nlay_ems)
c
c-----------------------------------------------------------------------
...
c
      ...
      integer       ispc, num_emsfiles, idxfile, itmp, jtmp, isoa
      integer       ibgdat, iendat, iounit, idx, iseg, i, j
      integer       ichkems, ichktm1, ichktm2, icls, buffer_offset
      real          bgtim, edtim
      logical       lfound, use_this_file, use_this_species
c
...
c
            ...
            if( idx .LE. 0 ) goto 20
            use_this_species = lusespc(idx)
            if( luse_soap3 .and. lsoa ) then
                if( this_species(1:4) .EQ. 'POA ' .OR. 
     &                    this_species(1:4) .EQ. 'HOA ' ) 
     &                     this_species = soap_emiss_names(IDX_SOAP_POA_AV)
                if( this_species(1:4) .EQ. 'POA_') use_this_species = .TRUE.
            endif
            if( .NOT. use_this_species ) goto 20
c
...
c
            if( luse_soap3 .AND. lsoa ) then
               ...
               do isoa=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                  icls = icls + 1
                  if( this_species .EQ. soap_emiss_names(isoa) ) then
