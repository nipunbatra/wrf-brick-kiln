      subroutine sum1pnt(numcols,numrows,nspmod,nspmod_tot,nsptrac,
     &                igroup,idxfile,idx,this_species,numpts,emsbas,
     *                  emsoth,emslft,emstot,emspnt,emssum,izcel,idcompact,lemit)
      use grid
      use chmstry
      use pigsty
      use filunit
...
c        nspmod_mod        number of model species including SOAP emissions species
...
c
      ...
      integer   idx
      integer   numpts
      real*8    emslft(numcols,numrows,nspmod_tot)
      real*8    emstot(numcols,numrows,nspmod_tot)
      real      emspnt(MXPTSRC)
