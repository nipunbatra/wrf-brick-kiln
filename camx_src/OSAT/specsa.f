...
c
      ...
      integer      ibegdt, ienddt, icls, ispc
      integer      ncount, ioff, idtnow, nhours, numic_spcs, numbc_spcs
      integer      numtc_spcs, num_icbc_specs, i, j, k, l
...
c
      clsnam(ITRAG2) = 'AG2'
      lgasflg(ITRAG2) = .TRUE.
      molwt_cls(ITRAG2) = 150.0
c
      clsnam(ITRAG1) = 'AG1'
      lgasflg(ITRAG1) = .TRUE.
      molwt_cls(ITRAG1) = 150.0
c
      clsnam(ITRBG2) = 'BG2'
      lgasflg(ITRBG2) = .TRUE.
      molwt_cls(ITRBG2) = 180.0
c
      clsnam(ITRBG1) = 'BG1'
      lgasflg(ITRBG1) = .TRUE.
      molwt_cls(ITRBG1) = 180.0
c
      clsnam(ITRPA2) = 'PA2'
      lgasflg(ITRPA2) = .FALSE.
c
      clsnam(ITRPA1) = 'PA1'
      lgasflg(ITRPA1) = .FALSE.
c
      clsnam(ITRPB2) = 'PB2'
      lgasflg(ITRPB2) = .FALSE.
c
      clsnam(ITRPB1) = 'PB1'
      lgasflg(ITRPB1) = .FALSE.
c
      clsnam(ITRPA0) = 'PA0'
      lgasflg(ITRPA0) = .FALSE.
c
      clsnam(ITRPB0) = 'PB0'
      lgasflg(ITRPB0) = .FALSE.
c
      clsnam(ITRIVA) = 'IVA'
      lgasflg(ITRIVA) = .TRUE.
      molwt_cls(ITRIVA) = 212.0
c
      clsnam(ITRSVA) = 'SVA'
      lgasflg(ITRSVA) = .TRUE.
      molwt_cls(ITRSVA) = 220.0
c
      clsnam(ITRPEC) = 'PEC'
      lgasflg(ITRPEC) = .FALSE.
c
      clsnam(ITRHOA) = 'HOA'
      lgasflg(ITRHOA) = .FALSE.
c
      clsnam(ITRPFC) = 'PFC'
...
c
c  --- if doing SOAP3, update the tracer species list with SOAP2
c      emissions species ---
c
      if( luse_soap3 .AND. lsoa ) then
         if( ngroup .EQ. 0 ) then
             ioff = 0
             do i=1,nregin
                do k=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                    write(name,'(A,I3.3,I3.3)') soap3_clsnam(k),1,i
                    ptname(soap3_iptcls(k)+ioff) = name
                enddo
                ioff = ioff + 1
             enddo
         else
             ...
             do j=1,ncount
                do i=1,nregin
                   do k=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                      write(name,'(A,I3.3,I3.3)') soap3_clsnam(k),j,i
                      ptname(soap3_iptcls(k)+ioff) = name
                   enddo
                   ioff = ioff + 1
                enddo
