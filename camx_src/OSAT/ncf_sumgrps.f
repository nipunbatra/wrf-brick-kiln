...
c
      ...
      integer       this_date, iounit, igroup, this_tstep, isoa
...
c
                 do ispc=1,nspmod
                     if( ispc .LE. nspec ) then
                       this_var = spname(ispc)
                       do isoa=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                         if( this_var .EQ. soap_emiss_names(isoa) ) 
     &                               this_var = soap_emiss_names(isoa)
                       enddo
                     else
                       this_var = soap_emiss_names(ispc-nspec-1+IDX_SOAP_POA_OP)
                     endif
...
c
     ...
     &                             nspec,nspmod,nsptrac,igroup,igrid,ispc,this_var,
     &                               emssum,emsgrd,emsbas,emsoth,emslft,emstot,lemit)
                 enddo
c
...
c
     ...
     &                                                       le1day,.TRUE.)
               ...
               data_count2(2) = 1
c
c  ---- loop over the list of emissions species ---
c
               do ispc=1,nspmod
                 if( ispc .LE. nspec ) then
                     this_var = spname(ispc)
     ...
     &                                               data_count2,emspnt)
                 call sum1pnt(numcols,numrows,nspec,nspmod,nsptrac,igroup,idxfile,
     &                  ispc,this_var,nptsrc,emsbas,emsoth,emslft,emstot,emspnt,
