...
c
      ...
      integer       num_ptsfiles, idx_start, isoa, iclass
      real          bgtim, edtim, rdum
      logical       lfound, lfirst_pass, luse_this_species
      logical       luse_model_species
c
...
c
              do i=1,npoint
                idxcp = i
                if( lcompactpt(igroup,idxfile) ) idxcp = idcompact(i)
                ptemis(idxcp+idx_start,idx) = emspts_in(i)/(60.*dtems)
              enddo
c
c  --- skip the rest if not an SA species ---
c
              luse_this_species = .FALSE.
              luse_model_species = .FALSE.
              if( lusespc(idx) ) then
                 luse_this_species = .TRUE.
                 luse_model_species = .TRUE.
              endif
              do isoa=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                if( luse_soap3 .AND. lsoa .AND.
     &             ptspcname .EQ. soap_emiss_names(isoa) ) 
     &                               luse_this_species = .TRUE.
              enddo
              if( luse_soap3 .and. lsoa ) then
                if( ptspcname(1:4) .EQ. 'POA ' .OR. 
     &                    ptspcname(1:4) .EQ. 'HOA ' ) 
     &                     ptspcname = soap_emiss_names(IDX_SOAP_POA_AV)
                if( ptspcname(1:4) .EQ. 'POA_') luse_this_species = .TRUE.
              endif
              if( .NOT. luse_this_species ) goto 10
c
...
c
                if( luse_model_species ) 
     &             ptemis(i+idx_start,idx) = emspts_in(i)/(60.*dtems)
c
...
c
   ...
   20            continue
                 ...
                 do isoa=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                     iclass = iclass + 1
                     if( lpigsa(idxcp+idx_start) ) cycle
                     if( ptspcname .EQ. soap_emiss_names(isoa) ) then
                        emscls(iclass,idxcp+idx_start) = 
     &                      emscls(iclass,idxcp+idx_start) + emspts(idxcp)
                     endif
                 enddo
              enddo
c
