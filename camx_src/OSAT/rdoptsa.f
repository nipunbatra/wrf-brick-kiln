...
c
 7001 continue
     ...
     &           'source apportionment types: OSAT/APCA/PSAT'
      ...
      write(iout,'(1X,A)') '    SA_Treat_SOAP3_Class'
...
c
 7005 continue
      write(iout,'(//,a)') 'ERROR in RDOPTSA:'
      write(iout,'(2a)') 'You requested the SOAP3 class treatment in',
     &                        ' the Source Apportionment (PSAT).'
      write(iout,'(2a)') 'But the chemistry parameters file does not ',
     &                                        'include SOAP3.'
      write(iout,'(2a/,a)') 'Please run with a SOAP3 chemparam file or',
     &  ' turn off treatment of SOAP3 class in ','the SA_Control namelist.'
c
 7006 continue
      write(iout,'(//,a)') 'ERROR in RDOPTSA:'
      write(iout,'(2a)') 'You requested the SOAP3 class treatment in',
     &                        ' the Source Apportionment (PSAT).'
      write(iout,'(2a)') 'But you did not select the SOA class.'
      write(iout,'(2a/,a)') 'This is not allowed. PLease turn on SOA class.'
      call camxerr()
c
c-----------------------------------------------------------------------
c    Return point:
