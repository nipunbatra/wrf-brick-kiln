...
c     07/06/21   --gy--       Revised Mechanism 7 for CB7r1
c     09/03/21   --gy--       Added Mechanism 9 (RACM2s21)
...
c
     ...
     &           + cold(kbrn3) + cold(kbrn2) + cold(kcln3) + cold(kcln2)
        delRGN = delcon(2,kno2) + delcon(2,kno3) + 2.*delcon(2,kn2o5)
     &           + delcon(2,kino3) + delcon(2,kbrn3) + delcon(2,kbrn2)
     &             + delcon(2,kcln3) + delcon(2,kcln2)
...
c
     ...
     &                                                + delcon(2,kino3)
        delNIT = delcon(2,kno) + delcon(2,khono)
        cycNIT = MIN( 0.9999, 1. - EXP(-rrxn_irr(1)/sumRGN) )
        alphaNIT = MAX( 0.0, rrxn_irr(90) + 0.23*rrxn_irr(197) +
     &                 0.082*rrxn_irr(149) + 0.14*rrxn_irr(153) +
     &                   0.14*rrxn_irr(157) + 0.02*rrxn_irr(192) +
     &                      0.26*rrxn_irr(203) + 0.4*rrxn_irr(209) )
        if( (cold(kntr1) + cold(kntr2) + cold(kcron)) .GT. FUZZ*convfac*
     &                  (bdnl(kntr1) + bdnl(kntr2) + bdnl(kcron)) ) then
           alphaNTR = rrxn_irr(95) + rrxn_irr(96) + rrxn_irr(174)
           betaNTR  = rrxn_irr(97)
        endif
     ...
     &                                                   /cold(kpna)) )
           ...
           lossINTR = cold(kintr) *
     &         (1. - EXP(-0.5*rrxn_irr(195)/cold(kintr)) )
           prodINTR = cold(kno2) * (1. - EXP(-0.1*rrxn_irr(179)/cold(kno2)) )
           cycINTR = 0.5 * AMIN1(lossINTR, prodINTR)
...
c
      elseif( idmech .EQ. 9 ) then
        ...
        delNIT = delcon(2,kno) + delcon(2,khono)
        cycNIT = MIN( 0.9999, 1. - EXP(-rrxn_irr(1)/sumRGN) )
        alphaNIT = MAX( 0.0, rrxn_irr(209) + rrxn_irr(210) +
     &                              0.12*rrxn_irr(128) + rrxn_irr(211) )

        if( (cold(konit) + cold(kison) + cold(knald)) .GT. FUZZ*convfac*
     &                  (bdnl(konit) + bdnl(kison) + bdnl(knald)) ) then
           alphaNTR = rrxn_irr(31) + rrxn_irr(123)
           betaNTR  = 0.0
        endif

        alphaHN3 = rrxn_irr(8) + rrxn_irr(57)

        if( cold(khno3) .LT. FUZZ*convfac*bdnl(khno3) ) alphaHN3 = 0.0

        gammaRGN = MAX( 0.0, delcon(4,kHNO3) - alphaTPN - betaNTR
     &                                                    + alphaHN3 )
        sumTPN = cold(kpan) + cold(kppn)
     &                   + cold(kmpan) + cold(khno4)
        delTPN = delcon(2,kpan) + delcon(2,kppn)
     &                   + delcon(2,kmpan) + delcon(2,khno4)

        if( sumTPN .GT. FUZZ*convfac*(bdnl(kpan) + bdnl(kppn)
     &                          + bdnl(kmpan) + bdnl(khno4) ) ) then
           alphaTPN = 0.0
           gammaTPN = 0.0
           lossPAN = cold(kpan) *
     &         (1. - EXP(-(rrxn_irr(32)+rrxn_irr(33)+
     &                        rrxn_irr(120)+rrxn_irr(167))/cold(kpan)) )
           prodPAN = cold(kno2) * (1. - EXP(-rrxn_irr(166)/cold(kno2)) )
           cycPAN = 0.5 * MIN(lossPAN, prodPAN)

           lossPPN = cold(kppn) *
     &            (1. - EXP(-(rrxn_irr(121)+rrxn_irr(169))/cold(kppn)) )
           prodPPN = cold(kno2) * (1. - EXP(-rrxn_irr(168)/cold(kno2)) )
           cycPPN = 0.5 * MIN(lossPPN, prodPPN)

           lossMPAN = cold(kmpan) *
     &         (1. - EXP(-(rrxn_irr(122)+
     &                       rrxn_irr(159)+rrxn_irr(171))/cold(kmpan)) )
           prodMPAN = cold(kno2) * (1. - EXP(-rrxn_irr(170)/cold(kno2)))
           cycMPAN = 0.5 * MIN(lossMPAN, prodMPAN)

           lossHNO4 = cold(kpna) *
     &         (1. - EXP(-(rrxn_irr(9)+rrxn_irr(67)+rrxn_irr(68))
     &                                                    /cold(kpna)) )
           prodHNO4 = cold(kno2) * (1. - EXP(-rrxn_irr(66)/cold(kno2)) )
           cycHNO4 = 0.5 * MIN(lossHNO4, prodHNO4)

           cycTPN = cycPAN * cold(kpan)/sumTPN +
     &                cycPPN * cold(kppn)/sumTPN +
     &                  cycMPAN * cold(kmpan)/sumTPN +
     &                    cycHNO4 * cold(khno4)/sumTPN 

        endif

        alphaRGN = MAX( 0.0, delcon(2,konit) + delcon(2,kison) +
     &                            delcon(2,knald)
     &                                 - alphaNIT - alphaTPN + betaNTR )
c
