...
c
      ...
      integer ndate, icel, jcel, num_ptsfiles, isoa
...
c
            if( is_netcdf_file ) then
               call ncf_rdptgrp(igroup,emscls,izcel)
            else
               call rdptgrp(ndate,ttime*100.,igroup,emscls,izcel)
            endif
c
...
c
                  ...
                  if( luse_soap3 .AND. lsoa ) then
                     ...
                     do isoa=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                        iclass = iclass + 1
                        if( igroup .EQ. 0 ) then
                            lefcls(ntrcls+isoa,idxbase) = emscls(iclass,idxbase)
                        else
                            lefcls(iclass,idxbase) = 
     &                         lefcls(iclass,idxbase) - emscls(iclass,idxbase)
c
c  --- put emissions into position in gridded tracer emissions array ---
c
                            ipos = soap3_iemcls(isoa) - 1 + imap + (igroup-1)*nregin
                            sapnts(idxbase,ipos) = emscls(iclass,idxbase) * frac
                        endif
                     enddo
                  endif
c
