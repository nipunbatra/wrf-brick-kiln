C***** NCF_RDPTGRP
c
      subroutine ncf_rdptgrp(igroup,emscls,izcel)
      use chmstry
      use filunit
      use ptemiss
      use tracer
...
c
      ...
      integer       idx_poa_xx(IDX_SOAP_POA_OP:IDX_SOAP_POA_BB), this_idx
...
c
      ...
      if( luse_soap3 ) then
         do isoa=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
            do ispc = 1,nemspc
               if( emspcname(ispc) .EQ. soap_emiss_names(isoa) )
     &                                       idx_poa_xx(isoa) = ispc
            enddo
         enddo
       endif
c
...
c
     ...
     &                                               data_count,emispts_in)
             if( ierr .NE. NF_NOERR) goto 7001
             if( idx_poa_xx(isoa) .LE. 0 ) cycle
c
c  --- put into regular model array ---
c
              do idxpt=1,nptsrc_safile(igroup,idxfile)
                ptemis(idxpt+idx_start,idx_poa_xx(isoa)) = emispts_in(idxpt)/(60.*dtems)
              enddo
c
c  --- if not needed for source apportionment skip the rest ----
c
...
c
                if( lpigsa(idxpt+idx_start) ) cycle
                emscls(icls,idxpt+idx_start) =
     &                   emscls(icls,idxpt+idx_start) + emispts_in(idxpt)/(60.*dtems)
             enddo
           enddo
         endif
