...
c
      ...
      integer ncount, ioff, icls, itrc, soap3_nspec
      logical lerror
...
c
      soap3_nspec = nspec
      if( luse_soap3 .AND. lsoa ) soap3_nspec = 
     &                 nspec+(IDX_SOAP_POA_BB-IDX_SOAP_POA_OP+1)
      
      allocate( emssum(soap3_nspec,soap3_ntotsp) )
      allocate( emsbas(soap3_nspec,soap3_ntotsp) )
      allocate( emsoth(soap3_nspec,soap3_ntotsp) )
c
      ncola = maxval( ncol(1:ngrid) )
      nrowa = maxval( nrow(1:ngrid) )
      allocate( emslft(ncola,nrowa,soap3_nspec) )
      allocate( emstot(ncola,nrowa,soap3_nspec) )
c
...
c
      call sumgrps(ncola,nrowa,soap3_nspec,soap3_nsaspc,ndlast,ttlast*100.,emstot,
     &                           emslft,emsbas,emsoth,emssum,lemit)
      call ncf_sumgrps(ncola,nrowa,soap3_nspec,soap3_nsaspc,ndlast,ttlast,emstot,
     &                           emslft,emsbas,emsoth,emssum,lemit)
c
...
c
     ...
     &                        emslft(i,j,idx)) / emstot(i,j,idx) )
               ...
               if( emstot(i,j,idx) .LE. 0. .AND. emslft(i,j,idx) .GT. 0. ) then
                  diff = 100.
                  if( diff .GT. difmax ) then
                      imax = i
                      jmax = j
     ...
     &                                             'emissions table.'
             if( idx .LE. nspec ) then
                 write(iout,'(/,A,A)') 'Species: ',spname(idx)
             else
                 write(iout,'(/,A,A)') 'Species: ',soap_emiss_names(idx-nspec+IDX_SOAP_POA_OP-1)
             endif
...
c
  ...
  10    continue
      ...
      do icls=ntrcls+1,soap3_ntrcls
        ...
        idx = idx + 1 
        do i=soap3_iptcls(iclass),soap3_nptcls(iclass)
c
c   --- ignore if this is an initial condition or boundary condition
c       tracer ---
c
           if( ptname(i)(7:8) .EQ. 'IC' ) cycle
           if( ptname(i)(7:8) .EQ. 'BC' ) cycle
c
c   --- find the class for this species ---
c
           sumtrc = 0.
           ...
           yldhvoc = 0.
           yldlvoc = 0.
           tonems(i) = tonems(i) + emssum(idx,i) / CVTTON
        enddo
      enddo
c
...
c
     ...
     &                      tonems(itrc) + diff * mwspec(idx) / CVTTON
         ...
         do icls=ntrcls+1,soap3_ntrcls
            ...
            idx = idx + 1 
            do i=1,nregin
               itrc = soap3_iemcls(iclass) - 1 + i + ngroup*nregin
               diff = emsbas(idx,itrc) - emsoth(idx,itrc)
               tonems(itrc) = tonems(itrc) + diff / CVTTON
                wtkoh(itrc) = 0.
                wtmir(itrc) = 0.
                yhrates(itrc) = 0.
                ylrates(itrc) = 0.
...
c
     ...
     &        idxcls(icls) .EQ. ITROOV ) CYCLE 
          ...
          iclass = icls
          if( icls .GT. ntrcls ) iclass = icls - ntrcls + (IDX_SOAP_POA_OP-1)
          do i=ioff,ncount
             if( lemit(icls) ) then
     ...
     &                  '     Average ','Emissions' 
                ...
                do j=1,nregin
                   if( i .GT. 0 ) then
                      ...
                      if( icls .GT. ntrcls ) itrc = soap3_iemcls(iclass) - 1 + j + (i-1)*nregin
                   else
                      itrc = iemcls(icls) - 1 + j
                      if( icls .GT. ntrcls ) itrc = soap3_iemcls(iclass) - 1 + j
                   endif
                   if( icls .LE. ntrcls ) then
