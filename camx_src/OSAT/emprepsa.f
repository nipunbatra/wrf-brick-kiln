...
c
      ...
      integer       ifile, nseg, nspect, igroup, isoa
...
c
     ...
     &                                            namein = 'FORM      '
                   do isoa=1,NUM_SOAP_SPECIES
                        if( namein(1:4) .EQ. 'POA ' ) cycle
                        if( namein .EQ. soap2_spec_names(isoa) )
     &                            namein = soap3_spec_names(isoa)
                   enddo
                   if( namein(1:4) .EQ. 'HOA ') namein = 'POA       '
                   if( namein(1:4) .EQ. 'POA ' .AND. luse_soap3 ) namein = 'POA_AV    '
                   do j=1,nspec
                   ...
                   if( luse_soap3 .AND. lsoa ) then
                      do j=IDX_SOAP_POA_OP,IDX_SOAP_POA_BB
                        if(namein .EQ. soap_emiss_names(j)) then
                           idxems(igrid,igroup,idxfile,ispc) = j
                           goto 20
                        endif
                      enddo
